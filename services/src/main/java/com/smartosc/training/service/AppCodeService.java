...
public class AppCodeService {
    ...
    public AppCodeService(AppCodeRepository appCodeRepository, ModelMapper modelMapper) {
    ...
    //add new app code
    public AppCodeDTO addAppCode(AppCodeDTO appCodeDTO) {
        try {
            ...
            if (appCodeRepository.findByCode(appCodeDTO.getCode()) == null) {
                ...
                throw new FieldDuplicateException("Khong Add Duoc");
            }
        } catch (Exception e) {
            logger.error(e.getMessage());
            throw new RuntimeException(e.getMessage());
